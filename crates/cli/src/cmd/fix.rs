...
fn write_suppress_entries(issues: &[Issue], project_root: &Path, dry_run: bool) -> Result<()> {
    ...
    // pyproject-mode = pyllow.toml doesn't exist AND pyproject.toml has a
    // `[tool.pyllow]` table. In that mode the loader only honors suppress
    // entries nested as `[[tool.pyllow.suppress]]`; emitting top-level
    // `[[suppress]]` here would invite a copy-paste that pyllow silently
    // ignores.
    let pyproject_mode = !config_path.exists() && pyproject_has_tool_pyllow(&pyproject_path)?;
    let (table_header, target_path) = if pyproject_mode {
        ("[[tool.pyllow.suppress]]", &pyproject_path)
    } else {
        ("[[suppress]]", &config_path)
    };
    let body = build_suppress_body(&entries, table_header);
    ...
    if pyproject_mode {
        bail!(
            "project is configured via [tool.pyllow] in pyproject.toml; \
             ...
             Re-run with --dry-run to preview the {n} entr{plural} (already \
             formatted as [[tool.pyllow.suppress]]) and paste them into \
             pyproject.toml",
            n = entries.len(),
...
/// Render collected suppression entries as TOML text. `table_header` is the
/// full bracketed header (e.g., `[[suppress]]` for top-level pyllow.toml,
/// `[[tool.pyllow.suppress]]` for pyproject-nested configs). The pyproject
/// form is critical for dry-run previews of pyproject-configured projects:
/// the loader ignores top-level `[[suppress]]` there, so copy-pasting a
/// preview using the wrong header silently does nothing.
fn build_suppress_body(entries: &[(String, String, Option<u32>)], table_header: &str) -> String {
...
mod tests {
    ...
    fn body_uses_top_level_header_for_pyllow_toml_mode() {
        ...
        assert!(!body.contains("[[tool.pyllow.suppress]]"));
    }

    #[test]
    fn body_uses_qualified_header_for_pyproject_mode() {
        // Pi P2 regression: dry-run output in pyproject mode must already
